...
from ..mots_common.io import load_sequences, load_seqmap, LABELS
...
def visualize_sequences(seq_id, tracks, max_frames_seq, img_folder, output_folder, draw_boxes=False, create_video=True):
  ...
  frame_list = sorted(list(os.listdir(img_folder + "/" + seq_id)))
  for t in range(max_frames_seq + 1):
    print("Processing frame", t)
    filename_t = img_folder + "/" + seq_id + "/" + frame_list[t]
    ...
    if t in tracks:
      for obj in tracks[t]:
        ...
        if obj.class_id < len(LABELS):
          category_name = LABELS[obj.class_id]
        else:
