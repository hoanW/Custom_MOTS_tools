...
import sys
from ..mots_common.io import load_seqmap, load_sequences, IGNORE_CLASS, LABELS
...
def run_eval(results_folder, gt_folder, seqmap_filename):
  ...
  print("Compute tracking eval with simplified matching and MOTSA")
  
  results_class = [None] * (len(LABELS)-1)
  for i in range(1, len(LABELS)):
    print("Evaluate class: " + LABELS[i])
    results_class[i-1] = evaluate_class(gt, results, max_frames, i)
